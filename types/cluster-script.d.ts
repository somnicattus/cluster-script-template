...
interface ClusterScript {
  ...
  onReceive(
    ...
    option?: { player: boolean; item: boolean },
  ): void;
...
/** @internal @item */
export interface GroupStateProxy {
  [propName: string]: Sendable;
}
 ...
 * @player
 */
type PlayerScriptSendablePrims =
  | Vector2
  | Vector3
  | Quaternion
  | PlayerId
 ...
 * @player @beta
 */
type IconAsset = NonNullable<unknown>;
...
/** @internal */
export interface UnityComponentPropertyProxy {
  [propName: string]: UnityComponentPropertyValue;
}

/** @internal */
type UnityComponentPropertyValue =
  | boolean
  | number
  | string
  | Vector2
